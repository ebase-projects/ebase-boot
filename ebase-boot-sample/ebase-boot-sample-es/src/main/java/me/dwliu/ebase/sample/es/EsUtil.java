...
public class EsUtil {
	...
	public static SearchSourceBuilder buildGeoHashGridSearch(boolean beidou, boolean satellite, boolean ais, boolean mmsi) {
		...
		if (beidou || satellite || ais || mmsi) {
			boolQueryBuilder = buildGeoQuery(beidou, satellite, ais, mmsi, -1, -1, -1);
		}
		searchSourceBuilder.query(boolQueryBuilder);
...
//        ScriptSortBuilder builder = new ScriptSortBuilder(script, ScriptSortBuilder.ScriptSortType.NUMBER);
	...
	public static BoolQueryBuilder buildGeoQuery(boolean beidou, boolean satellite, boolean ais, boolean mmsi, double lon, double lat, double radius) {
		...
		if (beidou || satellite || ais || mmsi) {
			if (beidou) {
				devBoolQueryBuilder
					.mustNot(QueryBuilders.termQuery("BEIDOU", ""))
					.must(QueryBuilders.existsQuery("BEIDOU"));
			}
			if (satellite) {
				devBoolQueryBuilder
					.should(QueryBuilders.boolQuery()
						.mustNot(QueryBuilders.termQuery("SATELLITE", ""))
						.must(QueryBuilders.existsQuery("SATELLITE")));
			}
			if (ais) {
				devBoolQueryBuilder
					.should(QueryBuilders.boolQuery()
						.mustNot(QueryBuilders.termQuery("AIS", ""))
						.must(QueryBuilders.existsQuery("AIS")));
			}
			if (mmsi) {
				devBoolQueryBuilder
					.should(QueryBuilders.boolQuery()
						.mustNot(QueryBuilders.termQuery("MMSI", ""))
						.must(QueryBuilders.existsQuery("MMSI")));
			}
		}
