...
public class CustomJsonValidateLoginTokenFilter extends OncePerRequestFilter {
	...
	private final AuthenticationEntryPoint authenticationEntryPoint = new CustomJsonAuthenticationEntryPoint();
...
//					Authentication authentication = jwtTokenUtils.getAuthentication(token, userInfoDetails);
			...
			JwtTokenException jwtTokenException = new JwtTokenException(e.getMessage());
			this.authenticationEntryPoint.commence(request, response, jwtTokenException);
			return;
		}
		// 继续下一个过滤器
		filterChain.doFilter(request, response);
