...
public class QiniuOssTemplate implements OssWithBucketTemplate {
        ...
        if (buckets != null && buckets.length > 0) {
            ...
            if (!list.contains(bucketName)) {
                ...
                bucketManager.createBucket(getBucketName(bucketName),
                        StringUtils.isBlank(properties.getQiniu().getZone()) ? Zone.autoZone().getRegion() : properties.getQiniu().getZone());
    ...
    public String fileUrl(String bucketName, String fileName) {
        return properties.getQiniu().getEndpoint().concat("/").concat(fileName);
    }
    ...
    public String getBucketName() {
        return getBucketName(properties.getQiniu().getBucketName());
    }
